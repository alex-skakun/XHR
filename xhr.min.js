!function (a) {
    ...
    function b (a, b) {
        ...
        for (h = 0, i = k.length; i > h; h++) {
            f = k[h].toLowerCase(), j[f] = c.defaultHeaders[f];
        }
        if ("object" == typeof b) {
            for (d = Object.keys(b), h = 0, i = d.length; i > h; h++) {
                f = d[h].toLowerCase(), j[f] =
                    b[f];
            }
        }
        for (e = Object.keys(j), h = 0, i = e.length; i > h; h++) {
            f = e[h], g = j[f], isNaN(g) ||
            "undefined" == typeof g || null === g || a.setRequestHeader(f, String(g))
        }
    }
    ...
    function c (d) {
        if (d) {
            ...
            return e.addEventListener("error", function (a) {f.applyCallback("error", a)}), e.addEventListener(
                ...
                e.status >= 200 && e.status < 400 ? f.applyCallback("success", b) :
                e.status >= 400 && e.status < 600 && f.applyCallback("error", a)
    ...
    function b (a) {
        ...
        this.silent = !1, this.callbacks = {
            ...
            getXHR: function () {return a}
        }
    }

    b.prototype.applyCallback = function (a, b) {
